...
% Parameters
Ca = .005; %.00175;%.003;   %Attractive Strength
Cr = .005; %Repuslive Strength (non-matching chromosome)
Cr1 = .005; %Repuslive Strength for matching chromosome enforcing body size
...
cpair(:,1) = tt1;
for q = 1:13
cpair(q,2) = sum(chrom_pair(:,q))/sum(chrom_total(:,q));
end
dlmwrite(['chrom_pair_v_time_IL_alltime.dat'],cpair,'delimiter','\t','precision',5)
...
dlmwrite(['chrom_pair_v_size_IL_9hr.dat'],cpair,'delimiter','\t','precision',5)
